...
export function ChatBubble({ message, isMe }: ChatBubbleProps) {
  ...
  const { user } = useAuth(); // We need participant info, which isn't on the message object
  ...
  // The user object of the sender is not available on the message itself.
  // We'll need to fetch it or pass it down if we want to show avatars for every message.
  // For simplicity, we'll hide the avatar on the bubble for now.
